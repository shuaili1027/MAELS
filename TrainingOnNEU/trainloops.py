...
class trainloop():
    def __init__(self, params):
        ...
        if self.params.stage == "stage2":
            ...
            self.D.load_state_dict(torch.load(self.params.stage1_path)["D"])
    ...
    def stage2(self, loader, current_epoch, c_opt, d_opt, info_opt, en_opt, eg_opt, g_opt):
        ...
        for idx, (data, label) in enumerate(loader):
            ...
            for i in range(1):
                ...
                adv_loss = - 2.5 * torch.log(1-torch.exp(- self.cate_loss(self.target[0](x_fake), label_inputs)))
                ssim_loss = 0.01 * self.ssim_loss(x_fake, x_real)

                info_loss = g_loss3 + g_loss2 + adv_loss + ssim_loss
                info_loss.backward()
                info_opt.step()
